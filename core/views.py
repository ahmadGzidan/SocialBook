...
def upload(request):
    if request.method == "POST":
        ...
        # Get the uploaded image and caption from the form
...
def like_post(request):
    ...
    if like_filter is None:
        # The user hasn't liked the post, so create a new like
...
def signup(request):
    if request.method == "POST":
        ...
        if password == password2:
            ...
            messages.info(request, 'The passwords you provided did not match.')
            return redirect('signup')
...
def signin(request):
    if request.method=="POST":
        ...
        if user is not None:
            ...
            messages.info(request,'the credentials are invalid')
